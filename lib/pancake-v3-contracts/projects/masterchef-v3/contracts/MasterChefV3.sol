...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 cakePerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            cakePerSecond =
                (latestPeriodCakePerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending CAKE rewards.
    /// @dev The pending cake amount is based on the last state in LMPool. The actual amount will happen whenever
    /// liquidity changes or harvest.
        ...
        if (positionInfo.pid != 0) {
            ...
            if (address(LMPool) != address(0)) {
                ...
                if (
                    ...
                    reward =
                        ((rewardGrowthInside - positionInfo.rewardGrowthInside) * positionInfo.boostLiquidity) / Q128;
                }
            }
        ...
        returns (bytes4)
    {
        ...
        (,, cache.token0, cache.token1, cache.fee, cache.tickLower, cache.tickUpper, cache.liquidity,,,,) =
    ...
    function withdraw(uint256 _tokenId, address _to) external nonReentrant returns (uint256 reward) {
        ...
        if (address(LMPool) != address(0) && !emergency) {
            ...
            emit UpdateLiquidity(
                msg.sender, pid, _tokenId, liquidityDelta, positionInfo.tickLower, positionInfo.tickUpper
            );
        }
    ...
    /// @dev Warning!!! Please make sure to use multicall to call unwrapWETH9 or sweepToken when set recipient
    /// address(0), or you will lose your funds.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        if (_token == address(CAKE)) {
            unchecked {
                // In fact balance should always be greater than or equal to cakeAmountBelongToMC, but in order to avoid
                // any unknown issue, we added this check.

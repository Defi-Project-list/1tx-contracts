...
contract TestPancakeV3Router is IPancakeV3SwapCallback {
    ...
    function pancakeV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) public override {
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IPancakeV3Pool(msg.sender).token0() : IPancakeV3Pool(msg.sender).token1();

...
contract PancakeV3Pool is IPancakeV3Pool {
    ...
    function initialize(uint160 sqrtPriceX96) external override {
        ...
        slot0 = Slot0({
            ...
            feeProtocol: 209_718_400, // default value for all pools, 3200:3200, store 2 uint32 inside
            unlocked: true
        });

        if (fee == 100) {
            slot0.feeProtocol = 216_272_100; // value for 3300:3300, store 2 uint32 inside
        } else if (fee == 500) {
            slot0.feeProtocol = 222_825_800; // value for 3400:3400, store 2 uint32 inside
        } else if (fee == 2500) {
            slot0.feeProtocol = 209_718_400; // value for 3200:3200, store 2 uint32 inside
        } else if (fee == 10_000) {
            slot0.feeProtocol = 209_718_400; // value for 3200:3200, store 2 uint32 inside
        }

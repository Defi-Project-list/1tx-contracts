    ...
    function clear(Tree storage tree) internal {
        ...
        assembly {
            ...
            for { let i := 1 } iszero(gt(i, totalNodes)) { i := add(i, 1) } {
                let ptr := or(nodes, i)
                if iszero(shr(_BITPOS_PACKED_VALUE, sload(ptr))) { sstore(or(ptr, _BIT_FULL_VALUE_SLOT), 0) }
                sstore(ptr, 0)
            }
            sstore(nodes, 0)
        }
        ...
        assembly {
            ...
            function remove(nodes_, key_) -> err_ {
                ...
                if iszero(eq(cursor_, key_)) {
                    ...
                    sstore(or(nodes_, cursor_), xor(cursorPacked_, and(xor(packed_, cursorPacked_), m_)))

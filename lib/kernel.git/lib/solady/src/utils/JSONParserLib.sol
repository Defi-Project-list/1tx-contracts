...
library JSONParserLib {
    ...
    function at(Item memory item, uint256 i) internal pure returns (Item memory result) {
        ...
        assembly {
            ...
            if iszero(and(lt(i, mload(r)), eq(and(mload(item), _BITMASK_TYPE), TYPE_ARRAY))) { result := 0x60 } // Reset
                // to the zero pointer.
        }
    }
    ...
    function decodeString(string memory s) internal pure returns (string memory result) {
        ...
        assembly {
            ...
            if iszero(and(gt(n, 1), eq(0x2222, or(and(0xff00, mload(add(s, 2))), chr(end))))) { fail() } // Fail if not
                // double-quoted.
    ...
    function _query(bytes32 input, uint256 mode) private pure returns (bytes32 result) {
        ...
        assembly {
            ...
            function skip0To9s(pIn_, end_, atLeastOne_) -> _pOut {
                for { _pOut := pIn_ } 1 { _pOut := add(_pOut, 1) } { if iszero(lt(sub(chr(_pOut), 48), 10)) { break } } // Not
                    // '0'..'9'.

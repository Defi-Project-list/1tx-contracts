 ...
 *
 * Summarizing the above, the prioritization hierarchy is `setChain` -> `foundry.toml` -> environment variable ->
 * defaults.
 */
        ...
        returns (Chain memory)
    {
        if (bytes(chain.rpcUrl).length == 0) {
            try vm.rpcUrl(chainAlias) returns (string memory configRpcUrl) {
                ...
                // The upstream error thrown by forge for failing cheats changed so we check both the old and new
                // versions

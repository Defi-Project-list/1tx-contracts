...
contract ERC4337Test is SoladyTest {
    ...
    // (e.g. `keccak256("Parent(bytes32 childHash,Mail child)Mail(Person from,Person to,string contents)Person(string
    // name,address wallet)")`).
    ...
    function testIsValidSignature() public {
        ...
        bytes memory signature = abi.encodePacked(t.r, t.s, t.v, _PARENT_TYPEHASH, _DOMAIN_SEP_B, t.hash);
        ...
        signature = abi.encodePacked(t.r, t.s, t.v, uint256(_PARENT_TYPEHASH) ^ 1, _DOMAIN_SEP_B, t.hash);
        assertEq(account.isValidSignature(_toChildHash(t.hash), signature), bytes4(0xffffffff));

        signature = abi.encodePacked(t.r, t.s, t.v, _PARENT_TYPEHASH, uint256(_DOMAIN_SEP_B) ^ 1, t.hash);
        assertEq(account.isValidSignature(_toChildHash(t.hash), signature), bytes4(0xffffffff));

        signature = abi.encodePacked(t.r, t.s, t.v, _PARENT_TYPEHASH, _DOMAIN_SEP_B, uint256(t.hash) ^ 1);
        assertEq(account.isValidSignature(_toChildHash(t.hash), signature), bytes4(0xffffffff));

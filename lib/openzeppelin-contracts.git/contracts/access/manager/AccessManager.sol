 ...
 *
 * By default every address is member of the `PUBLIC_ROLE` and every target function is restricted to the `ADMIN_ROLE`
 ...
 *
 * Any account can be added and removed into any number of these roles by using the {grantRole} and {revokeRole}
 * functions
...
contract AccessManager is Context, Multicall, IAccessManager {
    ...
    function cancel(address caller, address target, bytes calldata data) public virtual returns (uint32) {
        ...
        if (_schedules[operationId].timepoint == 0) {
            ...
            // calls can only be canceled by the account that scheduled them, a global admin, or by a guardian of the
            // required role.

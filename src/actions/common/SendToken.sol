...
contract SendToken is ActionBase {
        ...
        returns (bytes32 result)
    {
        ...
        params.tokenAddr = _parseParamAddr(params.tokenAddr, _paramMapping[0], _returnValues);
        params.to = _parseParamAddr(params.to, _paramMapping[1], _returnValues);
        params.amount = _parseParamUint(params.amount, _paramMapping[2], _returnValues);

        params.amount = _sendToken(params.tokenAddr, params.to, params.amount);

        result = bytes32(params.amount);

        bytes memory logData = abi.encode(_recipeId, result);

        emit ActionEvent("SendToken", logData);
    }
